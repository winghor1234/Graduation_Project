...
import { prisma } from "@/lib/prisma"
import { Prisma, PurchaseOrderStatus, PurchasePaymentStatus } from "@prisma/client"
import { BadRequestError, NotFoundError } from "@/utils/response"
import { CreatePurchaseOrderInput, PurchasePaymentSummary, UpdatePurchaseOrderInput } from "./purchase.type"
import { generatePurchaseCode } from "@/utils/generateCode"
...
const purchaseInclude = {
    ...
    purchase_details: {
        include: {
            product: true,
            variant: true, // ✅ ຕ້ອງ include variant ສະເໝີ
        },
    },
    import: true,
...
export const purchaseService = {
    ...
    async getPurchases(options: Prisma.PurchaseOrderFindManyArgs = {}) {
        ...
        return prisma.purchaseOrder.findMany({
            ...
            skip,
            take,
            orderBy: (orderBy as Prisma.PurchaseOrderOrderByWithRelationInput) ?? { createdAt: "desc" },
    ...
    async createPurchase(data: CreatePurchaseOrderInput, userId: string) {
        return prisma.$transaction(async (tx) => {
            ...
            // ✅ ກວດ variant ທຸກລາຍການ
            for (const item of data.purchase_details) {
            ...
            return tx.purchaseOrder.create({
                data: {
                    ...
                    purchase_details: {
                        create: data.purchase_details.map(item => ({
                            product_id: item.product_id,
                            variant_id: item.variant_id, // ✅ ຕ້ອງມີ
    ...
    async updatePurchase(id: string, data: UpdatePurchaseOrderInput) {
        return prisma.$transaction(async (tx) => {
            ...
            const existing = await tx.purchaseOrder.findUnique({
                where: { purchase_id: id },
                include: { import: true, purchase_details: true },
            })
            ...
            // ລຶບ details ເກົ່າ ແລ້ວສ້າງໃໝ່
            await tx.purchaseDetail.deleteMany({ where: { purchase_id: id } })
    ...
    async createPurchasePayment(id: string) {
        return prisma.$transaction(async (tx) => {
            ...
            if (purchase.payment_status === PurchasePaymentStatus.PAID) { // ✅ enum ຖືກ
                throw new BadRequestError("This purchase is already fully paid")
            }

            if (!purchase.import) {
                throw new BadRequestError("No import record found for this purchase")
            }

            const { ordered_total, actual_total } = calcActualTotal(purchase.purchase_details)
            const alreadyPaid = purchase.paid_amount ?? 0
            const remaining = actual_total - alreadyPaid
            ...
            const updated = await tx.purchaseOrder.update({
                where: { purchase_id: id },
                data: {
                    paid_amount: actual_total,
                    payment_status: PurchasePaymentStatus.PAID, // ✅ enum ຖືກ
                },
                include: purchaseInclude,
            })
    ...
    async getPurchasePaymentSummary(purchaseId: string): Promise<PurchasePaymentSummary> {
        const purchase = await prisma.purchaseOrder.findUnique({
            where: { purchase_id: purchaseId },
            include: { purchase_details: true, import: true },
        })
    ...
    async deletePurchase(id: string) {
        return prisma.$transaction(async (tx) => {
            ...
            await tx.purchaseOrder.delete({ where: { purchase_id: id } })
        })
    },
}