...
const checkoutSchema = z.object({
    ...
    district_id: z.string().min(1, "ກະລຸນາເລືອກເມືອງ"),
    branch_id: z.string().min(1, "ກະລຸນາເລືອກສາຂາ"),
    // ✅ validate File type ແລະ size ແທນ z.any()
    paymentSlip: z
        .instanceof(File)
        .optional()
        .refine(
...
export default function CheckoutPage() {
    ...
    // ✅ redirect ກັບໄປ cart ຖ້າກະຕ່າວ່າງ
    useEffect(() => {
        if (cart.length === 0) router.push("/cart")
    }, [cart, router])

    // ✅ ໃໝ່ — redirect ໄປ login ຖ້າຍັງບໍ່ໄດ້ login
    useEffect(() => {
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            ...
                            <Card className="border shadow-sm bg-white">
                                ...
                                <CardContent>
                                    ...
                                    {/* ✅ ສະແດງ error ການ validate file */}
                                    {errors.paymentSlip && (
                                        <p className="text-xs text-destructive mt-2">{errors.paymentSlip.message}</p>
                                    )}
                                </CardContent>
                            </Card>
                        </div>
