...
const SHOE_SIZES = Array.from({ length: 31 }, (_, i) => String(i + 20))
        ...
        <div className="space-y-7">
            ...
            {isFiltered && (
                <button
                    type="button"
                    onClick={onReset}
                    className="w-full text-[11px] text-brand-muted hover:text-brand-white transition-colors tracking-wide pt-2"
                >
                    ລ້າງການຕັ້ງຄ່າທັງໝົດ
                </button>
            )}
        </div>
    )
